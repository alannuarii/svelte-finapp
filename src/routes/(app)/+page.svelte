<script>
	...
	if (dataBulanIni.length > 0) {
		for (let i = 0; i < dataBulanIni.length; i++) {
			...
			if (dataBulanIni[i].tarik === 1) {
				arrPemasukan.push(dataBulanIni[i].nilai);
			}
		}

		arrPengeluaran.forEach((num) => (totPengeluaran += num));
		arrPemasukan.forEach((num) => (totPemasukan += num));
	}
	...
	const saldo = totPemasukan + prevSaldo + totPengeluaran;
</script>
...
<section>
	...
	{:else}
		...
		<div class="mb-3">
			...
			<div class="row mb-1">
				<!-- svelte-ignore a11y-label-has-associated-control -->
				<label class="col-6">Total Pemasukan</label>
				<div class="col-6">
					<input
						type="text"
						class="form-control form-control-sm"
						value="Rp {totPemasukan.toLocaleString()}"
						disabled
					/>
				</div>
			</div>
							...
							<td class="item text-start">
								<div class="d-flex justify-content-between align-items-center">
									...
									<div>
										...
										/><ModalDelete id={data.id} item={data.item_keluar} />
									</div>
								</div>
							</td>
